  ...
  return (
    <div className="App">
      ...
      <Card className="AppBody">
        <CardHeader
          className="AppTitle"
          title={<span>{steps[activeStep].label}</span>}
        />
        ...
        <CardContent className="AppBodyContent">
          ...
          {activeStep === 1 && state.chain && state.wallet && (
            <CheckAllocation
              chain={state.chain}
              wallet={state.wallet}
              onCollectAllocation={handleCollectAllocation}
            />
          )}
          {activeStep === 2 &&
            state.chain &&
            state.wallet &&
            state.allocationResponse && (
              <ClaimAirdrop
                wallet={state.wallet}
                chain={state.chain}
        ...
        <CardActions
          className={`AppActions ${
            state.airdropClaimedSuccessfully || activeStep === 0
              ? "HiddenActions"
              : ""
          }`}
        >
